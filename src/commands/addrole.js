  ...
  execute: async (message, args) => {
    if (!args[0] || !args[1]) {
      const errorEmbed = createEmbed('error', {
        title: 'Erreur',
        description: 'Usage: `,addrole @membre @rôle`\nExemple: `,addrole @User @Membre`',
      });
    ...
    if (member.roles.cache.has(role.id)) {
      const errorEmbed = createEmbed('warning', {
        title: 'Information',
        description: `${member} possède déjà le rôle ${role}.`,
      });
      return message.reply({ embeds: [errorEmbed] });
    }
    ...
    try {
      ...
      const successEmbed = createEmbed('success', {
        title: 'Rôle ajouté',
        description: `Le rôle ${role} a été ajouté à ${member}.`,
      });
